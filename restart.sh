...
if param timbeta ; then
# Start timbeta
docker run --name timbeta -p 50000:5000 --link postgre:postgre -v /opt/tim-beta/:/service ${DAEMON_FLAG} -t -i tim:$(./get_latest_date.sh) /bin/bash -c "cd /service/timApp && source initenv.sh ; TIM_NAME=timbeta ; $TIM_SETTINGS python3 launch.py --with-gunicorn $END_SHELL"
fi
