...
BufferSim.prototype.update = function() {
    ...
    */
	...
	if ( (y-yp)*(y-yp) / (Math.min(y*y,yp*yp)+1e-15) < 1e-7 &&
	     (x-xp)*(x-xp) / (Math.min(x*x,xp*xp)+1e-15) < 1e-7 )
	     break;
    ...
    console.log("pH: " + (-Math.log(H+x+y)/Math.log(10.)));
    console.log("HA: " + (HA-x));
    console.log("A-: " + (A+x));


    this.data_points.push([H+x+y, OH+y, HA-x, A+x]);

    this.pH_text.html('pH = ' + toFixed(-Math.log10(H+x+y),2));
