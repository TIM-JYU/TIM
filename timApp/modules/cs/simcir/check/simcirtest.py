...
class Logik:
    ...
    def __init__(self, data):
        ...
        self.portit = (
            set()
        )  # Store the types of ports used in the schematic (ehkäpä lista olisi parempi, jos halutaan tallentaa myöhemmin määrää), vertailussa nykyään se kuitenkin muutetaan listaksi
        self.kytkematta = (
            []
        )  # Lista porteista joiden ulostulo(t) tai sisäänmeno(t) on kytkemättä. Ne joissa kaikki kytkemättä erotellaan erilleen
        self.kelluvat = (
            []
        )  # komponentit, joista ei mitään kytketty, eli ei vaikuta millään tavalla kytkentään.
        self.inputkytkematta = []  # Lista kytkimistä, joita ei ole kytketty
        self.porttiennenkytkinta = (
            []
        )  # lista porteista, jotka on DC:n ja kytminen välissä
        if data is not None:
            self.devices = {}
            self.ulostulot = []
            self.ledit = []
            ...
            for d in data["devices"]:
                ...
                if dev.dev_type == "Toggle":  # lisää muita input tyyppejä
                ...
                ):  # portit, joita kytkennässä käytetty
                    ...
                    if dev.dev_id in [
                        ...
                        self.kelluvat.append(
                            dev.dev_type
                        )  # ei yhtään input tai output kytketty
    ...
    def lausekkeesta_totuustaulu(self, lausekkeet):
        ...
        try:
                ...
                for (nimi, lauseke) in sorted(lausekkeet.items()):
                    ...
                    if lauseke == "x":  # jos LED ei kytketty
                        ...
                        try:
                            ...
                            return (
                                -6
                            )  ## jokin komponentti kytkennässä jota ei voida evaluoida
                        if callable(
                            arvo
                        ):  # Jos on funktio, niin LED:ltä ei johdotusta kytkimelle asti
                            return -8
            ...
            return (
                -4
            )  ## palautetaan tyhjä totuustaulu, kaaviosta puuttuu sisäänmenoja ja/tai ulostuloja
    ...
    def doit(self):
        ...
        for c in self.connectors:
            if (
                c.from_id == c.to_id
            ):  # Takaisinkytkentä (suoraan sisäänmenoon ulostulosta, helppo havaita, ei turhaan odoteta RecursionExce)
        ...
        if (
            self.kytkematta != []
        ):  # Jos jonkin komponentin (kaikki) inputit tai (kaikki) outputit kytkemättä
        ...
        for out in self.ulostulot:
            try:
                ...
                return (
                    -3,
                    self.portit,
                )  # Takaisinkytkentä (useamman komponentin kautta, ei helppo testata, joten napataan poikkeuksella)
...
def onko_virheita(
        ...
        if hyvportit == []:
            ...
            if vaaditutportit == []:
                ...
                return tulosta_vai_ei(
                    ...
                    + str(hyvportit)
                    + "\nvaan kytkennässä on myös portit "
                    + str(ylimportit)
                    + "\n",
                    tulosta,
                )
    ...
    if hyvportit != [] and vaaditutportit == []:
        return tulosta_vai_ei(
            0.0,
            virheet
            + "lisäksi kytkennässä ei ole yhtään sallituista porteista\n"
            + str(hyvportit)
            + ", siten että niiden sisäänmenot \nja ulostulot on kytketty.\n",
            tulosta,
        )
        ...
        return tulosta_vai_ei(
            ...
            + str(hyvportit)
            + "\nmutta kytkennässäsi on muita portteja "
            + str(ylimportit)
            + "\n",
            tulosta,
        )
...
def muotoile_lauseke(lauseke):
    ...
    )  # varmistetaan että sulkujen ympärillä on välejä
...
def testaa_kaaviot(
    ...
    )  # opettajan portit, ei käytetä, ehkä myöhemmin voisi antaa infoa opiskelijalle, jos käytti enemmän portteja kuin mallissa, mutta miten sen hoitaisi sitten kun opettaja antaa totuustaulun...
...
if __name__ == "__main__":
    ...
    try:
        ...
        sys.stdout.write(
            "Hyväksyttävät portit lista pitää olla lainausmerkeissä, jos siinä useampia portteja"
        )
        exit()
