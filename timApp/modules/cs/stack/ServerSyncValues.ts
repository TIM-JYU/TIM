function getClosestParentWithSelectorOrOptional(
    elem: Element | null,
    selector?: string,
    optional?: string
) {
    ...
    for (; elem; elem = elem.parentElement) {
        ...
        if (optional && elem.matches(optional)) {
            return elem;
        }
    }
    return null;
...
export function findParentElementFromScript(
    scriptId: string,
    parentTopSelector: string,
    ...
    try {
        let elem: HTMLElement | null =
            document.scripts[document.scripts.length - 1]; // document.currentScript does not work???
        if (!elem) {
            return document.querySelector(parentDivSelector) ?? document;
        }
        if (scriptId && scriptId != elem.id) {
            elem = document.getElementById(scriptId);
        }
        if (!elem) {
            return document.querySelector(parentDivSelector) ?? document;
        }
        return (
            getClosestParentWithSelectorOrOptional(
                elem,
                parentTopSelector,
                parentDivSelector
            ) ?? document
...
export class ServerSyncValues {
    ...
    constructor(
        ...
        this.jsxFrame = this.parentElem.querySelector(
            iframeSelector
        ) as HTMLIFrameElement;
        if (debugName) {
            this.debugElem = this.parentElem.querySelector(
                debugName
            ) as HTMLInputElement;
        }
        ...
        this.debug(
            "Start port initialize " + parentElement.id + " " + iframeSelector,
            "i"
        );
        ...
        this.jsxFrame.addEventListener("load", () => {
            ...
            this.jsxFrame.contentWindow!.postMessage(init, "*", [
                this.channel.port2,
            ]);
        });
    }
    ...
    onChange(e: CustomChange) {
        try {
            ...
            if (val === inp.lastValue) {
                return;
            }
            if (e.port == this.channel.port1) {
                return;
            } /* ei samaan kanavaan takaisin */
    ...
    getInput(name: string) {
        ...
        const input = this.parentElem.querySelector<CustomInput>(
            "#" + this.prefix + name
        );
        if (!input) {
            return null;
        ...
        input.addEventListener("input", (e) =>
            this.onChange(e as CustomChange)
        );
