...
type InputObj = {input: HTMLInputElement, lastValue: string, servname: string};
...
export class ServerSyncValues {
    ...
    constructor(parentElement: Element, iframeSelector: string, prefix: string, debugName: string, dname: string, opts?: IOptions) {
        ...
        if (debugName)
            this.debugElem = this.parentElem.querySelector(debugName) as HTMLInputElement;
    ...
    getInput(name: string) {
        ...
        if (inp) return inp;
        const input = this.parentElem.querySelector('#' + this.prefix + name) as CustomInput | null;
