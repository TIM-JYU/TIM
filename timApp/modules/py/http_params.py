...
class QueryParams:
    ...
    def __check_key(self, key: str) -> str:
        ...
        """
        key2 = "-" + key
        if key in self.__get_query:
            return key
        if key2 in self.__get_query:
            return key2
        if not self.__jso:
            return key
        if "input" in self.__jso and "markup" in self.__jso["input"]:
            if key in self.__jso["input"]["markup"]:
                return key
            if key2 in self.__jso["input"]["markup"]:
                return key2
        if "markup" not in self.__jso:
            return key
        if key in self.__jso["markup"]:
            return key
        if key2 in self.__jso["markup"]:
            return key2
        return key

    def get_param_from_json(self, key: str, default: object) -> object:  # return shoud be Any ???
        """Get param value from query's json part.  key may include . so that "state.userword" gives a value from inside
        "state" with key "userword".  If no match, then return default input is prefered over markup.
    ...
    def get_param(self, key: str, default: object) -> object:  # return shoud be Any ???
        """Get param value from query params that match to key.  If no match, then return default input is prefered over
        markup.
        ...
        """
        if key.find(".") >= 0:
            return self.get_param_from_json(key, default)
        ...
        if key not in self.__get_query:
            ...
            if key in self.__jso:
                return self.__jso[key]
            return dvalue
...
def post_params(request: http.server.BaseHTTPRequestHandler) -> QueryParams:
    """get POST params and get params from the request post params might me "normal" or json object.
