...
class PluginCore:
    ...
    def _prepare_chat_request(
        ...
        *,
        ...
        if caller_id != rule.owner:
            try:
                usage = LLMUsage.get_usage(rule, caller_id)
    ...
    def get_plugin_settings(
        ...
        llm_rule = LLMRule.get_llm_rule(document_id)
        if llm_rule is None:
        ...
        user_keys = LLMRule.get_user_api_keys(user_id)
        shared_keys = LLMRule.get_shared_api_keys(user_id)
        ...
        global_policy = LLMPolicy.get_global_policy(llm_rule)
    ...
    def save_instance(
        ...
        old_plugin_rule = LLMRule.get_llm_rule(document_id)

        # Need to create before checking any other input
        if old_plugin_rule is None:
            LLMRule.create_plugin(owner=caller_id, document_id=document_id)
    ...
    def delete_instance(
        ...
        if plugin_type is not None and plugin_type == "asktim":
            ...
            LLMRule.delete_llm_rule(caller_id, document_id)
    ...
    def get_user_data(self, caller_id: int, document_id: int) -> list[UserData]:
        ...
        usages = LLMUsage.get_usages(llm_rule)
        for usage in usages:
            user_id = usage.user
            ...
            user_policy_sql = LLMPolicy.get_user_policy(llm_rule, user_id)
    ...
    @staticmethod
    def _convert_sql_policy_to_generic_policy(sql_policy: LLMPolicy) -> GenericPolicy:
    ...
    def update_usage(
        ...
        """
        When caller_id is given the LLMUsage table of the user and LLMRule of the
        ...
        LLMUsage.update_instance_usage(rule, used_tokens)

        if user_id is not None:
            if timestamp is None:
                timestamp = ChatMessage.ts_ms()

            LLMUsage.update_usage(user_id, rule, used_tokens, timestamp)

        return True

    def _calculate_remaining_tokens(
        self, llm_rule: LLMRule, usage: LLMUsage | None, caller_id: int
    ) -> tuple[int | None, str]:
        """
        ...
        user_policy = LLMPolicy.get_user_policy(llm_rule, caller_id)
        global_policy = LLMPolicy.get_global_policy(llm_rule)
    ...
    def _calculate_usage_in_window(
        ...
        Assumes that the timestamps are in rising order within the LLMUsage table.
    ...
    @staticmethod
    def _policy_window_as_ms(policy: LLMPolicy | None) -> int:
        """Gives the duration of the time window of the given policy in milliseconds"""
        if not policy:
            raise TypeError("_policy_window_as_ms: LLMPolicy was None")
    ...
    def update_api_key_permissions(
        owner_id: int, public_key: str, groups: list[str], paths: list[str]
        ...
        key = LLMRule.update_api_key_permissions(
    ...
    def get_user_api_keys(self, owner_id: int) -> list[APIKey]:
        ...
        """
        rows = LLMRule.get_user_api_keys(owner_id)
        keys: list[APIKey] = []
        for row in rows:
    ...
    def try_access_api_key_row(
        user_id: int, public_key: str
    ) -> tuple[LLMRule, Provider, str]:
        key = LLMRule.access_api_key(user_id, public_key)
        if not key:
    ...
    @staticmethod
    def get_api_key(public_key: str) -> tuple[Provider, str]:
        """
        ...
        :return: A tuple (provider, api_key)
        """
        key = LLMRule.get_api_key_by_alias(public_key)
        if not key:
    ...
    @staticmethod
    def delete_api_key(owner_id: int, public_key: str) -> None:
        LLMRule.delete_api_key(owner_id, public_key)
