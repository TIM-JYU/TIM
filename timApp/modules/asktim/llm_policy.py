...
class LLMPolicy(db.Model):
    ...
    def set_policy(
        ...
        """
        Sets a new policy for the LLM rule. LLMPolicy can be a global policy for the whole LLM rule or a student policy
        for the given user.
        """
        if user:
            policy = LLMPolicy.get_user_policy(llm_rule, user)
        else:
            policy = LLMPolicy.get_global_policy(llm_rule)
        if not policy:
            policy = LLMPolicy(
                for_user=user,
    ...
    def delete_policy(policy_id: int) -> None:
        ...
        """
        stmt = delete(LLMPolicy).where(LLMPolicy.id == policy_id)
    ...
    def delete_global_policy(llm_rule: LLMRule) -> None:
        ...
        """
        stmt = delete(LLMPolicy).where(
            LLMPolicy.llm_rule_id == llm_rule.id,
            LLMPolicy.for_user == 0,
        )
        db.session.execute(stmt)
        db.session.commit()
    ...
    def delete_user_policy(llm_rule: LLMRule, user_id: int) -> None:
        ...
        stmt = delete(LLMPolicy).where(
            LLMPolicy.llm_rule_id == llm_rule.id,
            LLMPolicy.for_user == user_id,
        )
        db.session.execute(stmt)
        db.session.commit()

    @staticmethod
    def get_global_policy(llm_rule: LLMRule) -> LLMPolicy | None:
        """
    ...
    @staticmethod
    def get_user_policy(llm_rule: LLMRule, user_id: int) -> LLMPolicy | None:
        """
