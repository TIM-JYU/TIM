...
class LLMUsage(db.Model):
    ...
    def set_usage(
        ...
        :return: LLMUsage of the given user.
        """
        if token_usage_history is None:
            token_usage_history = []

        usage = LLMUsage.get_usage(llm_rule, user)
        if not usage:
            usage = LLMUsage(
                user=user,
    ...
    def update_usage(
        ...
        timestamp to usage history. New LLMUsage is created if one does not exist.
        :param user: ID of the user for the usage.
        :param llm_rule: LLM rule instance.
        :param used_tokens: Number of used tokens.
    ...
    @staticmethod
    def get_usage(llm_rule: LLMRule, user_id: int) -> LLMUsage | None:
        """
        Gets the usage of the given user in the given LLM rule instance.
        """
        stmt = select(LLMUsage).where(
            LLMUsage.llm_rule_id == llm_rule.id,
            LLMUsage.user == user_id,
        )
        return db.session.scalar(stmt)

    @staticmethod
    def get_usages(llm_rule: LLMRule) -> list[LLMUsage]:
        """
