...
import {TokenLimitForUser, UserLLMPolicyComponent} from "./userllmpolicy";
...
@Component({
    ...
    template: `
        ...
        <span class="glyphicon"
            ...
            <div class="settings-row">
                <button class="btn btn-link settings-section-btn"
                        (click)="globalLLMPolicyOpen = !globalLLMPolicyOpen">
                    <span class="glyphicon"
                          [class.glyphicon-chevron-right]="!globalLLMPolicyOpen"
                          [class.glyphicon-chevron-down]="globalLLMPolicyOpen">
                    </span>
                    Restrictions for all users:
                </button>

                <ng-container *ngIf="globalLLMPolicyOpen">
                    <userllmpolicy [userLimits]="tokenLimitAllUsers"
            ...
            <div class="settings-row">
                ...
                <usercontrol *ngIf="userControlOpen"
                             [setUserData]="userLLMUsageAndPolicyData"
                             (userDataRequest)="userDataRequest.emit()"
                             (llmPolicySaveRequest)="llmPolicySaveRequest.emit($event)"
                             [llmPolicySaveResponse]="llmPolicySaveResponse"
                >
                </usercontrol>
            </div>
...
export class ChatControlPanelComponent {
    ...
    filesOpen = false;
    promptOpen = false;
    globalLLMPolicyOpen = false;
    userControlOpen = false;

    @Input() llmPolicySaveResponse!: {
        result: string;
        error: string;
    };
    @Input() userLLMUsageAndPolicyData: undefined | UserData[];
    ...
    get invalidInputState(): boolean {
        return (
            ...
            this.invalidUserLLMPolicyState
        );
    }
