                    ...
                    <div class="asktim-body scroll-box" #conversationScroll>
                        ...
                        <div class="control-panel-container" *ngIf="isLoggedIn">
                            ...
                            <asktim-control-panel *ngIf="isTeacher"
                                                  ...
                                                  [tokenLimitAllUsers]="globalLLMPolicy"
                                                  [userLLMUsageAndPolicyData]="userLLMUsageAndPolicyData"
                                                  (userDataRequest)="getUserData()"
                                                  (llmPolicySaveRequest)="handleUserLLMPolicySave($event)"
    ...
    implements AfterViewInit, OnDestroy
{
    ...
    llmPolicySaveResponse = {
        result: "",
        error: "",
    };
    userLLMUsageAndPolicyData: undefined | UserData[] = [];
    ...
    globalLLMPolicy: TokenLimitForUser = {
    ...
    private setControlPanelData(data: ControlPanelData) {
        ...
        this.globalLLMPolicy = data.global_policy;
     ...
     **/
    async handleUserLLMPolicySave(userData: UserData) {
        if (!this.canSendRequest) {
            return;
        }
        this.isRunning = true;
        ...
        const response = await this.httpPost<{
            result: string;
            error: string;
        }>(this.route("saveUserLLMPolicy"), save_request);
        this.isRunning = false;
        if (response.ok) {
            const data = response.result;
            this.llmPolicySaveResponse = {
                error: data.error,
                result: data.result,
            };
        } else {
    ...
    async getUserData() {
        ...
        const response = await this.httpPost<{
            result: UserData[];
            error?: string;
        }>(this.route("getUserLLMPolicyData"), data_request);
        this.isRunning = false;
        if (response.ok) {
            const data = response.result;
            this.controlpanelError = data.error;
            this.userLLMUsageAndPolicyData = data.result;
        } else {
            this.controlpanelError = this.handleAngularError(response.result);
        }
    }
