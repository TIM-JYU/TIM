...
def add_events(events: list[CalendarEvent]) -> Response:
    ...
    for event in events:
        ...
        if booker_groups is not None:
            for booker_group_str in booker_groups:
                ...
                # event_bookers.append({event.id: booker_group_str})
                # bookers.append({"booker": booker_group_str, "event_id": event.id})
                # if booker_group_str not in bookers:
                #  bookers.append(booker_group_str)
        if setter_groups is not None:
            for setter_group_str in setter_groups:
                ...
                # event_setters.append({event.id: setter_group_str})
                # setters.append({"setter": setter_group_str, "event_id": event.id})
                # if setter_group_str not in setters:
                #   setters.append(setter_group_str)
        event_data = Event(
            title=event.title,
    ...
    # added_events: list[dict[str, Event]] = []
    # added_events: list[Event] = []
    # for event in events:
    ...
    for local_id in event_objs:
        ...
        for setter in event_setters[local_id]:
            ...
            # {
            #     "id": event_o["event"].event_id,
            #     "title": event_o["event"].title,
            #     "start": event_o["event"].start_time,
            #     "end": event_o["event"].end_time,
            #     "meta": {
            #         "signup_before": event_o["event"].signup_before,
            #         "enrollments": 0,
            #         "maxSize": event_o["event"].max_size,
            #         "location": event_o["event"].location,
            #         "message": event_o["event"].message,
            #     },
            # }
