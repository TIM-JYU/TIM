...
plugin_bp = Blueprint("plugin", __name__, url_prefix="")


def return_resource_response(resp):
    headers = {k: v for k, v in resp.headers.items() if k not in ("transfer-encoding",)}
...
def view_template(plugin, template, index):
    try:
        resp = call_plugin_resource(
            plugin, "template?file=" + template + "&idx=" + index
        )
        return return_resource_response(resp)
...
@plugin_bp.route(
    "/plugin/<plugintype>/<task_id_ext>/fetchExternal", methods=["PUT", "POST"]
)
def plugin_tid_call(plugintype: str, task_id_ext: str):
    ...
    plugin_response = call_plugin_generic(
        plugintype,
        request.method,
        "fetchExternal",
