...
def prepare_for_dumbo_attr_list_recursive(regex_obj: Pattern, plugin_data: dict) -> None:
...
def render_plugin_multi(doc: Document, plugin: str, plugin_data: List[Plugin],
    ...
    plugin_automd = plugin_reg.automd if plugin_reg.automd is not None else default_auto_md
...
def call_plugin_resource(plugin: str, filename: str, args: Any=None) -> requests.Response:
    try:
        plug = get_plugin(plugin)
