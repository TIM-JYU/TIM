...
def perform_migration(sqlite_path: str, postgre_path: str):
    ...
    # migrate_table(sq3c, pgc, 'message', id_column='msg_id', extra_clause='WHERE lecture_id IN (SELECT lecture_id FROM lecture WHERE doc_id IN (SELECT id FROM block))')
    # migrate_table(sq3c, pgc, 'newuser', id_column=None, placeholders={'created': "coalesce(%s, '2014-06-01 00:00:00'::timestamp)"})
