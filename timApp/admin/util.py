...
def process_items(func: Callable[[DocInfo, T], int], parser: ArgumentParser) -> None:
    ...
    with app.app_context():  # type: ignore[no-untyped-call]
        doc_to_fix = (
            DocEntry.find_by_path(opts.doc, fallback_to_id=True)
            if opts.doc is not None
            else None
        )
        folder_to_fix = (
            Folder.find_by_path(opts.folder, fallback_to_id=True)
        ...
        try:
            if getattr(opts, "dryrun", True):
                print(
                    "Dry run mode enabled - the following only shows what WOULD be done."
                )
            for d in docs:
                if opts.progress:
        ...
        if hasattr(opts, "dryrun"):
            print(
                f'Total paragraphs that {"would be" if opts.dryrun else "were"} '
