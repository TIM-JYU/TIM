...
class AccountImportTest(TimDbTest):
    def test_import_accounts(self):
        ...
        accounts = [
            UserInfo(
                email=f"{name}@example.com", full_name=name, username=f"{name}uname"
            )
        ...
        self.write_and_test(
            accounts, expected_existing=(name.username for name in accounts)
        )
        accounts = [
            UserInfo(email=f"{name}@example.com", full_name=name, username="")
            for name in [f"testimport_2_{i}" for i in range(0, num_accounts)]
        ]
        self.assertEqual(num_accounts, len(accounts))
    ...
    def test_email_existing(self):
        accounts = [
            UserInfo(email="test1@example.com", full_name="Real Name 1", username="t1")
        ]
        self.write_and_test(
            accounts, expected_existing=(name.username for name in accounts)
        )
    ...
    def write_and_test(
        self, accounts: list[UserInfo], username_is_email=False, expected_existing=None
    ):
    ...
    def test_import_no_password(self):
        csv_path = self.write_test_csv(
            [UserInfo(full_name="Testing User", username="t@example.com")]
        )
