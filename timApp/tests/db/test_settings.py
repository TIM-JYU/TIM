...
class SettingsTest(TimDbTest):
    def test_settings_reference(self):
        ...
        self.assertEqual(d.document.get_settings().get_dict(), d2.document.get_settings().get_dict())
    ...
    def test_settings_multiple_reference(self):
        ...
        p1 = doc2.add_paragraph_obj(DocSettings(doc2, YamlBlock(values={'a': '1'})).to_paragraph())
        p2 = doc2.add_paragraph_obj(DocSettings(doc2, YamlBlock(values={'b': '2'})).to_paragraph())
        doc.add_paragraph_obj(p1.create_reference(doc))
        doc.add_paragraph_obj(p2.create_reference(doc))
