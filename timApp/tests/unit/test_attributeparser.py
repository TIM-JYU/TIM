...
class AttributeParserTest(unittest.TestCase):
    def test_random(self):
        ...
        self.check_valid(
            "{#asd .someClass .someClass2}",
            {"taskId": "asd", "classes": ["someClass", "someClass2"]},
        )
        self.check_valid(
            "{#asd .someClass somekey=someval emptykey= .someClass2}",
            {
                "taskId": "asd",
    ...
    def test_keyvalues(self):
        ...
        self.check_valid('{somekey="someval"}', {"somekey": "someval"})
        self.check_valid(
            '{somekey="someval with spaces"}', {"somekey": "someval with spaces"}
        )
    ...
    def test_whitespace(self):
        ...
        self.check_valid(
            "``` {#asd plugin=csPlugin}   ", {"taskId": "asd", "plugin": "csPlugin"}, 4
        )
    ...
    def test_brace(self):
        self.check_valid(
            r'# Hey {Hey {math_type=svg math_preamble="\\usetikzlibrary{shapes}"}',
            {"math_type": "svg", "math_preamble": r"\usetikzlibrary{shapes}"},
            11,
        )
