...
class ParDiffTest(TimRouteTest):
    def test_par_diff(self):
        ...
        d = self.create_doc()
        self.get(
            f"/getParDiff/{d.id}/0/0",
            expect_content={"diff": [], "live": 0, "version": [0, 0]},
        )
        d.document.add_paragraph("1")
        r = self.get(f"/getParDiff/{d.id}/0/0")
        self.assert_dict_subset(r, {"live": 0, "version": [1, 0]})
        self.assert_dict_subset(r["diff"][0], {"after_id": None, "type": "insert"})
        self.assert_dict_subset(r["diff"][0]["content"], {"css": [], "js": []})
    ...
    def test_diff_normal_to_reference(self):
        ...
        d.document.update(
            f"""#- {{id="{par.get_id()}" rp="yyyy" rd="9999"}}\ntest""",
