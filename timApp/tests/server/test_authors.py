...
class AuthorsTest(TimRouteTest):
    def test_authors(self):
        ...
        self.assertEqual(authors, ['Logged-in users', 'user 1 Test', 'user 1 Test', 'user 1 Test'])
        self.post_par(d.document, 'edit', pars[1].get_id())
        authors = get_content(self.get(url, as_tree=True), username_selector)
        self.assertEqual(authors, ['Logged-in users', 'user 1 Test (2 edits)', 'user 1 Test', 'user 1 Test'])
        self.login_test2()
