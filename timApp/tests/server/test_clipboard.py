...
class ClipboardTest(TimRouteTest):
    def test_invalid(self):
        ...
        self.cut(d, pars[0], pars[0])
        self.paste(
            d,
            pars[0],
            expect_status=400,
            expect_content={
                "error": f"Document {d.id}: Paragraph not found: {pars[0].get_id()}"
            },
        )
    ...
    def test_copy_many(self):
        ...
        pars = d.document.get_paragraphs()
        mark_read(
            usergroup_id=self.current_group().id,
            doc=d.document,
            par=pars[0],
        ...
        self.assertEqual(
            ...
            len(
                list(
                    filter(
                        lambda r: r.type == ReadParagraphType.click_red, new_readings
                    )
                )
            ),
        )
        self.assertEqual(
            ...
            len(
                list(
                    filter(
                        lambda r: r.type == ReadParagraphType.hover_par, new_readings
                    )
                )
            ),
        )
        self.assertListEqual(
            ["test1", "test2", "test3", "test1", "test2", "test3", "test4"],
...
{d.document.export_markdown()}
            ...
            self.assertEqual(
                21 - abs(first_index - second_index) - 1, len(pars), msg=message
            )
