...
def upgrade():
    ...
    op.add_column(
        "messagelist", sa.Column("email_list_domain", sa.Text(), nullable=True)
    )
    op.add_column(
        "messagelist", sa.Column("removed", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "messagelist_distribution",
        sa.Column("channel", messagelist_distribution_enum, nullable=True),
    )
    op.drop_column("messagelist_distribution", "channel_id")
    op.add_column(
        "messagelist_member",
        sa.Column("membership_ended", sa.DateTime(timezone=True), nullable=True),
    )
    op.add_column(
        "messagelist_member",
        sa.Column("membership_verified", sa.DateTime(timezone=True), nullable=True),
    )
