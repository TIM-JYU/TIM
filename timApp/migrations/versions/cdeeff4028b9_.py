...
def upgrade():
    op.execute(
        """UPDATE block SET description = translation.doc_title
                  FROM translation
...
def downgrade():
    op.add_column(
        "translation",
        sa.Column("doc_title", sa.TEXT(), autoincrement=False, nullable=True),
    )
    op.execute(
        """UPDATE translation SET doc_title = block.description
                  FROM block
                  WHERE block.id = translation.doc_id"""
    )
