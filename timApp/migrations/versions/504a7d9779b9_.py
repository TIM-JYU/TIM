...
e = sa.Enum(
    "on_screen", "hover_par", "click_par", "click_red", name="readparagraphtype"
)
...
def upgrade():
    ...
    op.add_column(
        "readparagraphs",
        sa.Column(
            "type", e, nullable=False, server_default=ReadParagraphType.click_red.name
        ),
    )
    op.execute(
        "ALTER TABLE readparagraphs DROP CONSTRAINT IF EXISTS readparagraphs_pkey"
    )
    op.execute(
        """ALTER TABLE readparagraphs
