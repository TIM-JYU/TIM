...
memberjoinmethod_enum = sa.Enum(
    "DIRECT_ADD", "INVITED", "JOINED", name="memberjoinmethod"
)
...
def upgrade():
    ...
    op.add_column(
        "messagelist", sa.Column("allow_attachments", sa.Boolean(), nullable=True)
    )
    op.add_column(
        "messagelist", sa.Column("default_delivery_right", sa.Boolean(), nullable=True)
    )
    op.add_column(
        "messagelist",
        sa.Column("default_reply_type", replytolistchanges_enum, nullable=True),
    )
    op.add_column(
        "messagelist", sa.Column("default_send_right", sa.Boolean(), nullable=True)
    )
    op.add_column(
        "messagelist", sa.Column("non_member_message_pass", sa.Boolean(), nullable=True)
    )
    ...
    op.add_column(
        "messagelist", sa.Column("tim_user_can_join", sa.Boolean(), nullable=True)
    )
    op.add_column(
        "messagelist_distribution",
        sa.Column("message_list_id", sa.Integer(), nullable=True),
    )
    op.add_column(
        "messagelist_distribution", sa.Column("user_id", sa.Integer(), nullable=True)
    )
    op.drop_constraint(
        "messagelist_distribution_id_fkey",
        "messagelist_distribution",
        type_="foreignkey",
    )
    op.create_foreign_key(
        None, "messagelist_distribution", "messagelist", ["message_list_id"], ["id"]
    )
    op.create_foreign_key(
        None, "messagelist_distribution", "messagelist_member", ["user_id"], ["id"]
    )
    op.add_column(
        "messagelist_external_member",
        sa.Column("display_name", sa.Text(), nullable=True),
    )
    op.add_column(
        "messagelist_member",
        sa.Column("join_method", memberjoinmethod_enum, nullable=True),
    )
