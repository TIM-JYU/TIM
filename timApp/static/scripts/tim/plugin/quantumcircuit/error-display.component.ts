...
@Component({
    ...
    template: `
        ...
        <div *ngIf="error.kind === 'condition-not-satisfied'">
        ...
        <div *ngIf="error.kind === 'condition-not-interpretable'">
            <p i18n>Unable to interpret condition:</p>
            <p>{{error.condition}}</p>
        </div>

        <div *ngIf="error.kind === 'condition-invalid'">
            <p i18n>Condition has invalid characters:</p>
            <p>{{error.condition}}</p>
        </div>

        <div *ngIf="error.kind === 'matrix-incorrect'">
            <p i18n>Failed to parse matrix from input:</p>
            <p>{{error.matrix}}</p>
        </div>

        <div *ngIf="error.kind === 'answer-incorrect'">
