...
var editorScope_1 = require("tim/editorScope");
var timHelper = require("tim/timHelper");
var utils_1 = require("tim/utils");
var ngimport_1 = require("./ngimport");
utils_1.markAsUsed(angular_sanitize_1.default);
var imagexApp = angular_1.default.module("imagexApp", ["ngSanitize"]);
...
function drawText(ctx, s, x, y, font) {
...
function drawCirclesVideoDot(ctx, dr, videoPlayer) {
    for (var dri = 0; dri < dr.length; dri++) {
        ...
        var seg2 = 0;
        var s = "";
        for (var lni = 0; lni < seg.lines.length; lni++) {
        ...
        if (this.scope.lastDrawnSeg == seg2)
            return;
...
imagexApp.directiveTemplate = function () {
    ...
    if (imagexApp.TESTWITHOUTPLUGINS)
        return "";
...
imagexApp.initDrawing = function (scope, canvas) {
    ...
    function DragTask(canvas) {
        ...
        this.draw = function () {
            ...
            for (var i = 0; i < this.drawObjects.length; i++) {
                try {
                    ...
                    if (this.drawObjects[i].name == "dragobject") {
                        if (this.activeDragObject) {
                            var onTopOf = areObjectsOnTopOf(this.activeDragObject, this.drawObjects, "target");
                            if (onTopOf && onTopOf.objectCount < onTopOf.maxObjects) {
                                onTopOf.color = onTopOf.snapColor;
                            }
                            var onTopOfB = areObjectsOnTopOf(this.drawObjects[i], this.drawObjects, "target");
                            if (onTopOfB && this.drawObjects[i] !== this.activeDragObject)
                                onTopOfB.color = onTopOfB.dropColor;
                        }
                        else {
                            var onTopOfA = areObjectsOnTopOf(this.drawObjects[i], this.drawObjects, "target");
                            if (onTopOfA) {
            ...
            if (this.activeDragObject)
                this.activeDragObject.draw(this.activeDragObject);
        }.bind(this);
        ...
        this.downEvent = function (event, p) {
            ...
            if (this.canvas.coords && scope.preview) {
                ...
                if (typeof (editorScope_1.editorChangeValue) !== "undefined")
                    editorScope_1.editorChangeValue(["position:"], this.canvas.coords.value);
            }
        };
        ...
        this.upEvent = function (event, p) {
            if (this.activeDragObject) {
                ...
                if (event != p)
                    event.preventDefault();
                this.mousePosition = getPos(this.canvas, p);
                var isTarget = areObjectsOnTopOf(this.activeDragObject, this.drawObjects, "target");
        ...
        if (scope.freeHandShortCuts)
            ...
            this.canvas.addEventListener("keypress", function (event) {
                ...
                if (c == "r")
                    th.freeHand.setColor("#f00");
                if (c == "b")
                    th.freeHand.setColor("#00f");
                if (c == "y")
                    th.freeHand.setColor("#ff0");
                if (c == "g")
                    th.freeHand.setColor("#0f0");
                if (c == "+")
                    th.freeHand.incWidth(+1);
                if (c == "-")
                    th.freeHand.incWidth(-1);
                if (c == "1")
                    th.freeHand.setWidth(1);
                if (c == "2")
                    th.freeHand.setWidth(2);
                if (c == "3")
                    th.freeHand.setWidth(3);
                if (c == "4")
                    th.freeHand.setWidth(4);
                if (c == "l")
                    th.freeHand.flipLineMode();
    ...
    function isKeyDef(value, key, defaultValue) {
        ...
        for (var i = 0; i < keys.length - 1; i++) {
            var k_1 = keys[i];
            if (!p[k_1])
                p[k_1] = {}; // if not on p, add
            p = p[k_1]; // for next round
            if (v && v[k_1])
                v = v[k_1];
            else
                v = null;
        ...
        if (p && (typeof p["iskey" + k] !== "undefined"))
            return p["iskey" + k];
        if (d && (typeof d[k] !== "undefined"))
            return true;
        return ret;
    }
    ...
    function getValueDef(value, key, defaultValue, keepEmtpyAsNone) {
        if (keepEmtpyAsNone === void 0) { keepEmtpyAsNone = false; }
        var keys = key.split(".");
        var v = value;
        var p = scope.previousValue;
        var d = scope.defaults;
        ...
        for (var i = 0; i < keys.length - 1; i++) {
            var k_2 = keys[i];
            if (!p[k_2])
                p[k_2] = {}; // if not on p, add
            p = p[k_2]; // for next round
            if (v && v[k_2])
                v = v[k_2];
            else
                v = null;
        ...
        if (p && (typeof p[k] !== "undefined") && p[k] != null)
            return p[k];
        if (d && (typeof d[k] !== "undefined") && d[k] != null)
            return d[k];
        return ret;
    }
    ...
    var grabOffset;
    if (isTouch)
        grabOffset = scope.extraGrabAreaHeight;
    else
        grabOffset = 0;
    ...
    var shapeFunctions = {
        ...
        rectangle: {
            ...
            draw: function (objectValues) {
                ...
                this.ctx.arc(this.r1 / 2 - this.cornerRadius, -this.r2 / 2 + this.cornerRadius, this.cornerRadius, 1.5 * Math.PI, 0);
                this.ctx.lineTo(this.r1 / 2, this.r2 / 2 - this.cornerRadius);
                this.ctx.arc(this.r1 / 2 - this.cornerRadius, this.r2 / 2 - this.cornerRadius, this.cornerRadius, 0, 0.5 * Math.PI);
                this.ctx.lineTo(-this.r1 / 2 + this.cornerRadius, this.r2 / 2);
                this.ctx.arc(-this.r1 / 2 + this.cornerRadius, this.r2 / 2 - this.cornerRadius, this.cornerRadius, 0.5 * Math.PI, Math.PI);
                this.ctx.lineTo(-this.r1 / 2, -this.r2 / 2 + this.cornerRadius);
        ...
        vector: {
            ...
            draw: function (objectValues) {
                ...
                this.ctx.lineTo(this.r1 - this.arrowHeadLength, this.r2 / 2 - this.arrowHeadWidth / 2);
        ...
        img: {
            init: function (initValues) {
                ...
                if (this.imgproperties.textbox)
                    this.textbox.init(initValues);
        ...
        textbox: {
            init: function (initValues) {
                ...
                this.r1 = getValue(this.textBoxSize[0], this.textwidth + this.leftMargin + this.rightMargin);
            ...
            draw: function (objectValues) {
                ...
                this.ctx.arc(this.r1 - this.cornerRadius, this.cornerRadius, this.cornerRadius, 1.5 * Math.PI, 0);
                this.ctx.lineTo(this.r1, this.r2 - this.cornerRadius);
                this.ctx.arc(this.r1 - this.cornerRadius, this.r2 - this.cornerRadius, this.cornerRadius, 0, 0.5 * Math.PI);
                this.ctx.lineTo(this.cornerRadius, this.r2);
                this.ctx.arc(this.cornerRadius, this.r2 - this.cornerRadius, this.cornerRadius, 0.5 * Math.PI, Math.PI);
                this.ctx.lineTo(0, this.cornerRadius);
                this.ctx.arc(this.cornerRadius, this.cornerRadius, this.cornerRadius, Math.PI, 1.5 * Math.PI);
        ...
        pin: {
            init: function (initValues) {
                ...
                this.pinLength = getValueDef(initValues, "pin.length", (this.type === "vector") ? 0 : 15);
            ...
            init2: function () {
                ...
                if (this.type === "vector") {
                    this.pinPosition = getValue(this.pinPositions[this.pinPositionAlign], this.pinPositions.west);
                }
                else
                ...
                if (this.pinProperties && this.pinProperties.visible) {
                    ...
                    this.dotPosition = { x: this.pinPosition.x, y: this.pinPosition.y };
            },
            draw: function (objectValues) {
                this.ctx = objectValues.ctx;
                this.ctx.save();
    ...
    scope.error = "";
    if (userFixedObjects) {
        for (var i = 0; i < userFixedObjects.length; i++) {
    ...
    if (scope.attrs.state && scope.attrs.state.userAnswer) {
        ...
        if (objects && userDrags && userDrags.length > 0) {
            for (var i = 0; i < objects.length; i++) {
                if (!objects[i])
                    continue; // looks like the first may be null
...
imagexApp.initScope = function (scope, element, attrs) {
    ...
    */
    ...
    var vp = ngimport_1.$window.videoApp && videoApp.videos[scope.followid];
    if (vp) {
    ...
    var use = false;
    if (scope.freeHand == "use") {
        scope.freeHand = false;
        use = true;
    }
    ...
    if (scope.freeHandData)
        scope.freeHandDrawing.freeDrawing = scope.freeHandData;
    ...
    if (n > 1)
        element[0].childNodes[n - 1].outerHTML =
...
ImagexScope.prototype.getPColor = function () {
    if (this.scope.preview) {
        if (typeof (editorScope_1.editorChangeValue) !== "undefined") {
            ...
            editorScope_1.editorChangeValue(["[a-zA-Z]*[cC]olor[a-zA-Z]*:"], '"' + globalPreviewColor + '"');
        }
    }
};
...
ImagexScope.prototype.doshowAnswer = function () {
    ...
    $scope.http({ method: "PUT", url: url, data: params, headers: { "Content-Type": "application/json" }, timeout: 20000 }).success(function (data, status, headers, config) {
...
ImagexScope.prototype.videoPlay = function () {
    var video = this.scope.videoPlayer;
    if (video.fakeVideo)
        return;
    //    var $scope = this.scope;
