...
export class Document {
    ...
    public refreshSectionReadMarks() {
        ...
        for (const sectionPars of this.sections.values()) {
            const readlines = $(sectionPars.map((p) => p[0])).children(
                ".readline"
            );
            ...
            if (modifiedCount + unreadCount > 0) {
                sectionPars[sectionPars.length - 1].append(
                    $("<div>", {
                        class: "readsection",
                        title:
                            "Mark preceding section as read (" +
                            sectionPars.length +
                            " paragraphs - " +
                            unreadCount +
                            " unread, " +
                        ...
                        '<i class="glyphicon glyphicon-align-left"></i><i class="glyphicon glyphicon-ok"></i>'
                    )
                );
            }
        }
    ...
    private buildSections(
        currentSectionPars: Paragraph[],
        container: JQuery
    ): JQuery[] {
        let child = container.children(".par:first");
        while (child.length > 0) {
            if (child.hasClass("area")) {
                currentSectionPars = this.buildSections(
                    currentSectionPars,
                    child.find(".areaContent")
                );
            } else if (child.hasClass("par")) {
                const attrs = getParAttributes(child);
                const refAttrs = getRefAttrs(child)["ref-attrs"] as Record<
                    string,
                    string
                >;
                ...
                if (content.is(":visible")) {
                    if (content.children("h1, h2, h3").length > 0) {
                        ...
                        !attrs.hasOwnProperty("settings") &&
                        !attrs.hasOwnProperty("area") &&
                        !attrs.hasOwnProperty("area_end") &&
                        !refAttrs.hasOwnProperty("area") &&
                ...
                if (currentSectionPars.length > 0) {
                    this.sections.set(
                        getParId(
                            currentSectionPars[currentSectionPars.length - 1]
                        )!,
                        currentSectionPars
                    );
                }
