...
export function isArrowKey(ev: KeyboardEvent) {
    ...
    const keyCode = ev.keyCode; // deprecated but faster as long as it works
    switch (keyCode) {
        case KEY_DOWN:
        case KEY_UP:
        case KEY_LEFT:
...
export function keyCodeToString(keyCode: number): string {
    ...
    if (keyCode >= keyboardMap.length) {
        return "";
    }
    return keyboardMap[keyCode].toUpperCase();
}

export function stringToKeyCode(st: string): number {
    return keyboardMap.indexOf(st.toUpperCase());
}
...
export function isKeyString(ev: KeyboardEvent, st: string): boolean {
    ...
    // noinspection JSDeprecatedSymbols
    return ev.keyCode === stringToKeyCode(st.toUpperCase());
}
...
export function isKeyCode(ev: KeyboardEvent, keyCode: number): boolean {
    ...
    if (ev.keyCode !== undefined) {
        // this is deprecated, but a lot faster
        // noinspection JSDeprecatedSymbols
...
// TODO: this map may contain errors, at least ArrowDown etc was just ARROW and what to do with cases?
const keyboardMap = [
    "", // [0]
    "", // [1]
    "", // [2]
