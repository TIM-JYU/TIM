...
timApp.config([
    ...
    ($httpProvider: IHttpProvider) => {
        ...
        const hdrs = $httpProvider.defaults.headers.get as Record<
            string,
            string
        >;
...
timApp.filter("timpreciseduration", [
    "$filter",
    ($filter: IFilterService) => {
        return (duration: string) => {
...
timApp.directive("onSave", () => {
    return (scope, element, attrs) => {
        element.bind("keydown", (event) => {
            if (
                (event.ctrlKey &&
                    (event.which === KEY_ENTER || event.which === KEY_S)) ||
                (element[0] instanceof HTMLInputElement &&
                    event.which === KEY_ENTER)
            ) {
                scope.$apply(() => {
...
timApp.config([
    "$sanitizeProvider",
    ($sanitizeProvider: ISanitizeProvider) => {
