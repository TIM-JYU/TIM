...
class TimDbBase(object):
    ...
    def insertBlockToDb(self, name: Optional[str], owner_group_id: int, block_type: int) -> int:
        ...
        """
        b = Block(description=name, type_id=block_type, usergroup_id=owner_group_id)
        self.session.add(b)
        self.session.flush()
        block_id = b.id
        assert block_id != 0
