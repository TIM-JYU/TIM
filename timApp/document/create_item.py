...
def do_create_item(
    ...
    validate_item_and_create_intermediate_folders(
        item_path, item_type, parent_owner or owner_group, validation_rule
    )
    ...
    if item_type == BlockType.Document and app.config["BOOKMARKS_ENABLED"]:
        ...
        bms.add_bookmark(
            ...
            item.url_relative,
            move_to_top=True,
            limit=app.config["LAST_EDITED_BOOKMARK_LIMIT"],
        ).save_bookmarks()
...
def apply_template(item: DocInfo, template_name: Optional[str] = None):
    ...
    if template_name:
        matched_templates = list(
            filter(lambda t: t.short_name == template_name, templates)
        )
    if not matched_templates:
        matched_templates = list(
            filter(lambda t: t.short_name == FORCED_TEMPLATE_NAME, templates)
        )
    if matched_templates:
        template = matched_templates[0]
        item.document.update(
            template.document.export_markdown(), item.document.export_markdown()
        )


def copy_document_and_enum_translations(
    source: DocInfo, target: DocInfo, copy_uploads: bool
) -> Generator[tuple[DocInfo, DocInfo], None, None]:
    is_preamble = f"/{TEMPLATE_FOLDER_NAME}/{PREAMBLE_FOLDER_NAME}/" in source.path

    if copy_uploads:
        target.children.extend(
            source.children
        )  # required to retain rights to uploaded files

    # Copy tags except course code and subject.
    for tag in source.block.tags:
        if tag.type == TagType.Regular:
            target.block.tags.append(
    ...
    for tr in source.translations:  # type: Translation
        ...
        if not tr.is_original_translation:
            document = create_document_and_block(
    ...
    if copy_id:
        ...
        if vr.issues:
            raise RouteException(
                f"The following errors must be fixed before copying:\n{vr}"
            )
    ...
    if isinstance(item, DocInfo):
        if d:
            for tr, new_tr in copy_document_and_enum_translations(
                d, item, copy_uploads=copy_uploads
            ):
