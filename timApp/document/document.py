...
class Document:
    ...
    def __update_par_map(self):
        ...
        count_of_cached_pars = len(self.__par_cache or [])
        for i in range(0, count_of_cached_pars):
            curr_p = self.__par_cache[i]
            par_id = curr_p.get_id()
            prev_p = self.__par_cache[i - 1] if i > 0 else None
            next_p = self.__par_cache[i + 1] if i + 1 < count_of_cached_pars else None
        ...
        if not self.__is_incomplete_cache:
            self.single_par_cache.update(
                {p.get_id(): p for p in self.__par_cache or []}
            )
    ...
    def load_pars(self):
        ...
        if self.__preamble_should_include and self.__preamble_pars:
            self.__par_cache = list(self.__preamble_pars) + (self.__par_cache or [])
    ...
    def get_settings_pars(self) -> Generator[DocParagraph, None, None]:
        ...
        for p_id in self.get_par_ids(no_preamble=True):
            if DocParagraph.is_setting_attrs(self.get_attrs(p_id)):
    ...
    def set_settings(self, settings: dict | YamlBlock, force_new_par: bool = False):
        ...
        yaml_settings: YamlBlock | dict = settings
        new_par = DocSettings(self, yaml_settings).to_paragraph()
    ...
    def add_paragraph_obj(self, p: DocParagraph, update_meta=True) -> DocParagraph:
        ...
        :return: The same paragraph object, or None if it could not add.
    ...
    def get_paragraph_by_task_id(self, task_id: str) -> DocParagraph or None:
        taketime("get_paragraph_by_task_id")
    ...
    def _save_doc_lines(self):
        ...
        tmp_path.replace(path)  # atomic on POSIX + Windows (os.replace)
