            ...
            DocRef::Area(refdoc, ref ra) => match docs.load_area(refdoc, ra) {
                Ok(pars) => {
                    ...
                    PP::Area(preprocess_pars(
                        pars.into_iter().map(|p| CtxBlock::new(refdoc, p)),
                    ))
                }
    ...
    blocks
        ...
        .map(|mut pp| loop {
            pp = match pp {
                ...
                PP::DerefBlock(box PP::Ref(ref _ctxpar, ref docref), _) => PP::DerefBlock(
                ...
                PP::Area(pars) => return PP::Area(dereference_blocks(pars, docs, visited_blocks)),
                PP::Ref(ref _ctxpar, ref docref) => PP::DerefBlock(
                    Box::from(deref_single(docref.clone(), docs, visited_blocks)),
                    Box::from(pp),
                ),
...
fn preprocess_single(p: CtxBlock) -> PP {
    ...
    match (plugin, rd, rp, ra, s) {
        (_, Some(rd), Some(rp), _, _) => PP::Ref(p, DocRef::Par(DocId(rd), BlockId(rp))),
...
fn preprocess_pars(pars: impl IntoIterator<Item = CtxBlock>) -> Vec<PP> {
